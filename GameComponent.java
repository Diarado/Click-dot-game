...
public class GameComponent extends JPanel implements MouseListener {
    ...
    private void timeout() {
        ...
        if(isActive) {
            if(targetCount == maxTargets) stopGame();
            else {
                if(target.x <= this.getWidth() && target.y <= this.getHeight()) {
                    targetCount++;
    ...
    private static class Target {
        ...
        void paintDot(Graphics g) {
            ...
            if(isHit) g.setColor(Color.RED);
            else g.setColor(Color.BLUE);
            g.fillOval(x-radius,y-radius,30,30);
        }
        ...
        boolean checkHit(int cx, int cy) {
            ...
            if(!isHit && Math.sqrt((cx-x)*(cx-x) + (cy-y)*(cy-y)) <= radius) {
                isHit = true;
                return true;
            }
            return false;
